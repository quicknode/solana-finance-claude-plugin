...
- **Compute `(1 + r)^n` by exponentiation-by-squaring on a scaled integer - never `powf`, `exp`, or anything touching `e`.** `e` and `powf` are floats: non-deterministic and banned by the integers-only rule above. Hold `1.0` as a WAD-scaled integer (`ONE_WAD = 1_000_000_000_000_000_000` = 1e18, in `u128`), form the per-period factor `(1 + rate_per_period)` in the same scale, and raise it to the integer power of periods elapsed with square-and-multiply. This is the algorithm the SPL token-lending lineage ships (Save/Solend, Port: `(1 + rate/SLOTS_PER_YEAR).pow(slots_elapsed)`, whose period is wrong as *Accrue by the wall clock* below explains), and it is *exact* discrete compounding up to one truncation per multiply - error `O(log n)` ULPs, far below token granularity.
...
- **Accrue by the wall clock, not by slots.** Interest and funding are rates quoted per year or per hour, so the periods are seconds elapsed (`Clock::get()?.unix_timestamp - last_accrual_timestamp`) and the per-period rate is `annual_rate / SECONDS_PER_YEAR` (`365 * 86_400`). A slot count is exact as a count and a guess as a duration: a `SLOTS_PER_YEAR` divisor assumes a slot length the network changes by feature gate and does not deliver even between changes. The SPL token-lending lineage shows the cost. Its constant was written as `DEFAULT_TICKS_PER_SECOND / DEFAULT_TICKS_PER_SLOT * SECONDS_PER_DAY * 365`, whose integer division compiled to 2 slots a second (63,072,000 a year) on a network targeting 2.5, and Solend's fork hard-coded that result with a comment calling it the slot rate; halving the slot to 200 ms doubles what such a program charges with no line of it changed. The timestamp is leader-written but bounded: each block may move it only a little relative to the slots since its parent, and the next leader pulls it back, so an elapsed time is out by a second or two however long the span. The same bound means a halt does not arrive as one jump: after a restart the timestamp trails real time and catches up over the following blocks, so the halt's interest accrues during the catch-up rather than in the first block. A few seconds of interest against an annual rate is too small to move a liquidation, which is why freshness stays in slots (*Oracle freshness uses slots, not unix time*) and accrual does not. Never subtract into a negative: treat a timestamp at or before the stored one as zero elapsed and leave the stored stamp where it is. Early-return when zero seconds have elapsed. If the program also needs to know it was refreshed this slot, keep a separate `last_update_slot` for that check; it is freshness, not accrual.
- **Accrue lazily into a stored cumulative index, not per position.** Keep one `cumulative_borrow_rate` (and `last_accrual_timestamp`) on the market; multiply it by the compound factor on any touch; each position's owed amount is a ratio of indices. This is the universal Solana pattern (Save, Port, Kamino) - it makes accrual permissionless and O(1) per user.
...
- **A truncated binomial/Taylor series is an acceptable *compute* optimisation, not a default.** For long gaps, exact `pow` is `O(log n)` full-width multiplies; Aave and Kamino instead expand `(1 + b)^n ≈ 1 + nb + n(n-1)/2·b² + n(n-1)(n-2)/6·b³`, which is `O(1)` and accurate because the per-second rate `b` is microscopic (so `b⁴+` vanishes). If you do this, keep the same scaled-integer/checked discipline and ensure the truncation error rounds *against* the user (Aave's comment notes it deliberately undercharges borrowers / underpays suppliers).
...
- State accounts: `seeds` / `address` derivation, or a stored-address check from another bound account. Each framework implements it in its own way, and the way changes between versions: Anchor 2 puts `address = state.field` on the account being checked (`has_one` is deprecated there), Anchor 1 put `has_one = field` on the state account, and Quasar writes `has_one(field)` on the state account.
...
- **Every account the program creates lives at a PDA, unless it cannot.** A PDA's address follows from its seeds, so any client can find it without being told, the program signs for it with those seeds, and there is no private key to generate, sign with once, and throw away. An account at a public key the client generated has none of that: finding it later takes a stored address or an index, and creating it takes one more signer. Vaults, positions, configs, per-user records, and program-created mints all live at PDAs. This covers accounts the program creates or makes the client create for it; wallets, and the mints and token accounts a user brings, are not the program's to place.
- **Size is the usual reason an account can't live at a PDA.** A program can allocate at most 10,240 bytes (`MAX_PERMITTED_DATA_INCREASE`) when it creates an account through CPI. A larger account, such as a 180 KB order book, is created by the client with the System Program's `create_account`, which needs the new account's own signature, and a PDA has none to give. (Growing an account at a PDA 10 KB per instruction with `realloc` is the alternative, trading the extra signer for a series of setup instructions.) "The example I copied did it", "it saves a seed", and "the client already has a key pair" are not reasons.
- **An account at a generated public key says why, and the program binds it.** Comment the reason on its constraint (`// Not a PDA: at 180 KB it exceeds the 10 KB CPI allocation limit, so the client creates it`), record its address on a state account, and check it against that record in every instruction (see *Account Binding*). In Anchor, an `init` without `seeds` is such an account; in Quasar, an `init` without `address = X::seeds(..)` is. Either one without that comment is a finding.
- Tests derive every PDA the program creates. A `Keypair::new()` for a program account is correct only for an account the bullet above allows.
- Add `pub bump: u8` to every struct stored in an account at a PDA
