...
- **Every account the program creates lives at a PDA, unless it cannot.** A PDA's address follows from its seeds, so any client can find it without being told, the program signs for it with those seeds, and there is no private key to generate, sign with once, and throw away. An account at a public key the client generated has none of that: finding it later takes a stored address or an index, and creating it takes one more signer. Vaults, positions, configs, per-user records, and program-created mints are all PDAs. This covers accounts the program creates or makes the client create for it; wallets, and the mints and token accounts a user brings, are not the program's to place.
- **Size is the usual reason an account can't be a PDA.** A program can allocate at most 10,240 bytes (`MAX_PERMITTED_DATA_INCREASE`) when it creates an account through CPI. A larger account, such as a 180 KB order book, is created by the client with the System Program's `create_account`, which needs the new account's own signature, and a PDA has none to give. (Growing a PDA 10 KB per instruction with `realloc` is the alternative, trading the extra signer for a series of setup instructions.) "The example I copied did it", "it saves a seed", and "the client already has a key pair" are not reasons.
